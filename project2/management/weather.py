...
def take_city_name():
    city_list = []
    """take the city name from the table"""
...
def take_coordinates_of_city(city):
    """take the geographic coordinates to find a city name"""
    limit = 1
...
def adding_data_for_all_cities():
    """takes names of cities and add it to data base"""
    db.session.query(WeatherTable).delete()
    db.session.commit()
    cities = take_city_name()
    for idx in cities:
